...
%========================================================================
% sys_aug - OPEN LOOP SIMULATION (elevator step + turbulence noise)
%========================================================================
t = (0:0.001:5)';
d_c_step = ones(size(t));        % step on elevator command
n1_noise = 1000*randn(size(t));       % white noise on n1

u = [d_c_step, n1_noise];        % columns match sys_aug.InputName order: {'d_c','n1'}
y = lsim(sys_aug, u, t);         % columns match sys_aug.OutputName order: {'alpha','q'}

figure;
plot(t, y(:,1))
xlabel('Time (s)'); ylabel('\alpha (rad)')
title('sys\_aug - Angle of Attack Response (elevator step + turbulence)')
grid on

figure;
plot(t, y(:,2))
xlabel('Time (s)'); ylabel('q (rad/s)')
title('sys\_aug - Pitch Rate Response (elevator step + turbulence)')
grid on
